...
for i in range(opt.totalcount):
    MakeImage(pipe, opt.mode, eta, opt.prompt, opt.prompt_neg, opt.steps, opt.width, opt.height, opt.seed, opt.guidance_scale, "onnx", opt.img , opt.imgscale, opt.imgmask, opt.outpath)
    opt.seed = opt.seed + 1
